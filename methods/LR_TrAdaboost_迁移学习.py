"""
基于逻辑回归和TrAdaboost的金融风控冷启动迁移学习
样本分为训练样本和测试样本，训练样本分为：源训练集和目标训练集
源训练集的数据分布与测试样本分布存在差异，目标训练集与测试样本较为相似
目标:通过大量的源训练集和少量目标训练集，将学习到的经验迁移到测试集上
TrAdaboost的核心是每轮训练，减少错误分类的源域样本权重，同时增大错误分类的目标域的样本权重
"""
import pandas as pd  
